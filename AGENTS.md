...
Agents are in `.Codex/agents/` and MUST be invoked for tasks matching their expertise:
...
Detailed rules are in `.Codex/rules/` (loaded automatically when matching files are read):
- **Go style, errors, logging, SPDX headers**: `.Codex/rules/go-style.md`
- **CLI architecture**: `.Codex/rules/cli-commands.md`
- **Testing**: `.Codex/rules/testing.md`
- **Operator/CRDs**: `.Codex/rules/operator.md`
- **PR creation**: `.Codex/rules/pr-creation.md`

**Plan review**: Before presenting an implementation plan, review all applicable `.Codex/rules/` files for the languages and components involved. Plans must conform to existing conventions.
...
- Follow `.Codex/rules/pr-creation.md` and `.github/pull_request_template.md`
...
2. **Suggest codifying it** — identify which `.Codex/rules/` file or `.Codex/agents/` file it belongs in and propose the edit
3. **Offer to apply** with a one-line confirmation (e.g., "Want me to add this to `.Codex/rules/go-style.md`?")
...
**Personal vs team conventions**: Personal preferences (e.g., "I like verbose output") belong in `~/.Codex/` personal memory. Team-wide conventions (e.g., "always use `errors.Is()` for error checks") belong in `.Codex/rules/` so all team members benefit.

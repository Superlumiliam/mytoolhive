---
name: add-rule
description: Captures a team convention or best practice and adds it to the appropriate .Codex/rules/ or .Codex/agents/ file
---
...
Formalize a convention, best practice, or correction into the project's `.Codex/rules/` or `.Codex/agents/` files so it applies automatically for all team members.
...
- **Rules files** (`.Codex/rules/`): Auto-loaded based on `paths:` frontmatter globs when Codex touches matching files. These define the canonical conventions (style, testing patterns, error handling, etc.).
- **Agent files** (`.Codex/agents/`): Define agent-specific behavior — persona, review checklist, output format, workflow steps. Agents inherit the full conversation context (including AGENTS.md), so they already have access to all loaded rules. Agent files should *reference* rules (e.g., "Follows conventions in `.Codex/rules/testing.md`"), never restate them.

Match the convention to an existing file based on scope:

| Scope | Target file | What goes here |
|-------|------------|----------------|
| General Go code | `.Codex/rules/go-style.md` | Style, naming, error handling conventions |
| Test files | `.Codex/rules/testing.md` | Testing patterns, framework usage |
| CLI commands | `.Codex/rules/cli-commands.md` | CLI architecture, flag conventions |
| Kubernetes operator | `.Codex/rules/operator.md` | CRD, controller conventions |
| PR creation | `.Codex/rules/pr-creation.md` | PR format, review expectations |
| Agent workflow/persona | `.Codex/agents/<agent-name>.md` | Agent-specific behavior, checklists, output format |
...
3. **A one-line confirmation prompt**: "Add this rule to `.Codex/rules/testing.md`? (y/n)"
...
- **Too broad for one file**: If the convention spans multiple scopes, suggest adding it to AGENTS.md instead or splitting into multiple rule additions
- **Personal preference vs team convention**: If the rule sounds personal (e.g., "I prefer tabs"), ask: "Is this a team-wide convention or a personal preference? Personal preferences go in your `~/.Codex/` memory instead."

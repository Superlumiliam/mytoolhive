...
2. **At the end of the session** (or when the user asks to reflect), analyze the notes for patterns:
   ...
   - Propose concrete updates to this skill, the vmcp-review skill, or `.Codex/rules/` files based on what was learned
...
When suggesting code changes in review comments, check `.Codex/rules/` for project-specific patterns and conventions before writing code. Suggestions should follow the project's established style (e.g., the immediately-invoked function pattern for immutable assignment in Go). When requesting changes from external contributors, always provide concrete code examples showing the expected structure — don't just describe what you want in prose.
